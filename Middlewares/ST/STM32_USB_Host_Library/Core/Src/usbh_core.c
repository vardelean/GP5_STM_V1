  ...
  switch (phost->gState)
  {
    ...
    case HOST_CHECK_CLASS:
      ...
      else
      {
        ...
        USBH_UsrLog("Searching for class match:");
        USBH_UsrLog("  Interface[0] Class=0x%02X, SubClass=0x%02X", 
                    phost->device.CfgDesc.Itf_Desc[0].bInterfaceClass,
                    phost->device.CfgDesc.Itf_Desc[0].bInterfaceSubClass);
        USBH_UsrLog("  Registered classes=%d", phost->ClassNumber);
        
        for (idx = 0U; idx < USBH_MAX_NUM_SUPPORTED_CLASS; idx++)
        {
          if (phost->pClass[idx] != NULL)
          {
            USBH_UsrLog("  Class[%d]: Code=0x%02X, Name=%s", 
                        idx, phost->pClass[idx]->ClassCode, phost->pClass[idx]->Name);
          }
